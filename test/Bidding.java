...
public class Bidding {
    ...
    public static void main(String[] args) throws Exception {
        ...
        String baseAdd = "/home/user/bidding";

        Bidding main = new Bidding();
        main.connect("192.168.82.214");
        main.getSession();
        main.createSchema("bidding");
    ...
    public void fillEntityIds() {
        String q = "SELECT regions0id, categories0id FROM bidding.table12;";
        ResultSet results = session.execute(q);
        for (Row row : results) {
            entityIds.get("table12").add(row.getString(0) + ":" + row.getString(1));
        }
        String q1 = "SELECT users0id FROM bidding.table5;";
        ...
        q1 = "SELECT items0id FROM bidding.table3;";
        ...
        q1 = "SELECT bids0id FROM bidding.table22;";
        results1 = session.execute(q1);
        currentBidsNumber = -1;
        for (Row row : results1) {
        ...
        q1 = "SELECT comments0id FROM bidding.table8;";
        results1 = session.execute(q1);
        ...
        q1 = "SELECT buy_now0id FROM bidding.table9;";
        results1 = session.execute(q1);
        currentBuyNumber = -1;
        for (Row row : results1) {
    ...
    public void createSchema(String ksName) throws Exception {
        ...
        session.execute("CREATE TABLE IF NOT EXISTS  table9 (users0id text,buy_now0date text, items0id text, buy_now0id text, buy_now0qty text, primary KEY (users0id, items0id, buy_now0id, buy_now0date));");
    ...
    public Long insertRecord(String tablename, String feilds, String data) throws Exception {
        ...
        session.execute("INSERT INTO bidding." + tablename + " (" + feilds + ") " + "VALUES (" + data + ");");
    ...
    public void loadItemQueryToCassandra(int itemCount) throws Exception {
        for (int i = 0; i < itemCount; i++) {
            ...
            // insert into table 4
            feilds = "categories0id ,items0id, items0end_date ,items0buy_now, items0name, items0max_bid, items0quantity, items0reserve_price, items0nb_of_bids, items0start_date, items0description, items0initial_price";
            ...
            feilds = "regions0id, categories0id ,items0id, items0end_date, users0id ,items0buy_now, items0name, items0max_bid, items0quantity, items0reserve_price, items0nb_of_bids, items0start_date, items0description, items0initial_price";
            ...
            feilds = "items0id,categories0id,items0end_date";
    ...
    public Long InsertItemQuery() throws Exception {
        ...
        StringBuilder builderTbl19 = new StringBuilder();
        builderTbl19.append("'items0id").append(i).append("',");
        builderTbl19.append("'").append(regions0id).append("',");
        builderTbl19.append("'categories0id").append(catRand).append("',");
        builderTbl19.append("'users0id").append(userRand).append("',");
        builderTbl19.append("'items0end_date").append(i).append("'");
    ...
    public void loadBuyNowQueryToCassandra(int buyNumber) throws Exception {
        ...
        for (int i = 0; i < buyNumber; i++) {
            ...
            String feilds = "users0id, buy_now0date, items0id, buy_now0id, buy_now0qty";
    ...
    public Long insertBidsQuery(Map<String, String> map) throws Exception {
        ...
        builderTbl2.append("'users0id").append(userRand).append("',");
        builderTbl2.append("'").append(users0nickname).append("',");
        builderTbl2.append("'items0end_date").append(map.get("items0id").replaceAll("id", "end_date")).append("'");
...
//        System.out.println("UPDATE::::::::::::>>>>>>>>>>>>>>>>>" + query + "<<<<<<<<<<<<<<<<<");
    ...
    public Long updateItem(Map<String, String> fields, int index) {
        ...
        String q = "update bidding.table3 set " + tmp + " where items0id='" + where + "' AND items0end_date='" + items0end_date + "';";
        calculateAFM("StoreBid:table3", 1);
        times += (UpdateQuery(q));

        // table 4
        q = "select categories0id from bidding.stable15 where items0id = '" + where + "';";
        Tuple3<Long, List<Row>, Integer> tuple3 = execRecord(q);
        calculateAFM("StoreBid:stable15", tuple3._3());
        times += (tuple3._1());
        for (Row row : tuple3._2()) {
            String cat0id = row.getString("categories0id");
            q = "update bidding.table4 set " + tmp + " where items0id='" + where
                    + "' AND categories0id='" + cat0id + "' AND items0end_date='" + items0end_date + "';";
            times += (UpdateQuery(q));
            calculateAFM("StoreBid:table4", 1);
        }

        // table 12
        q = "select regions0id, categories0id, users0id from bidding.stable19 where items0id = '" + where + "';";
        tuple3 = execRecord(q);
        calculateAFM("StoreBid:stable19", tuple3._3());
        times += (tuple3._1());
        for (Row row : tuple3._2()) {
            q = "update bidding.table12 set " + tmp
                    + " where regions0id='" + row.getString("regions0id")
                    + "' AND categories0id='" + row.getString("categories0id")
                    + "' AND items0id='" + where
                    + "' AND users0id='" + row.getString("users0id")
                    + "' AND items0end_date='" + items0end_date + "';";
            times += (UpdateQuery(q));
            calculateAFM("StoreBid:table12", 1);
        }
        fields.put("items0end_date", items0end_date);
        fields.put("items0id", where);
    ...
    public Map<String, String> getRecord(String tblName, String field, String value) {
        ...
        String q = "SELECT * FROM bidding." + tblName + " WHERE  " + field + " = '" + value + "';";
        ResultSet results = session.execute(q);
...
//        printResultset(rows);
    ...
    public double readBrowseCategories(int number, BufferedWriter writer) throws Exception {
        String q1 = "select users0nickname, users0password from bidding.table5 where users0id ='users0id";
        String q2 = "select categories0id, categories0name from bidding.table1 where categories0dummy = '0'";
        double sum = 0;
    ...
    public double readViewBidHistory(int number, BufferedWriter writer) throws Exception {
        String q1 = "select items0name from bidding.table3 where items0id = 'items0id";
        String q2 = "select users0id, users0nickname, bids0id from bidding.table2 where items0id = 'items0id";
        String q3 = "select bids0qty, bids0bid, bids0date from bidding.table22 where bids0id = ";
        double sum = 0;
        for (int i = 0; i < number; i++) {
    ...
    public double readViewItem(int number, BufferedWriter writer) throws Exception {
        String q1 = "select * from bidding.table3 where items0id = 'items0id";
        String q2 = "select bids0id from bidding.table2 where items0id = 'items0id";
        String q3 = "select bids0qty, bids0bid, bids0date from bidding.table22 where bids0id= ";
        double sum = 0;
        for (int i = 0; i < number; i++) {
            long d = 0;
    ...
    public double readSearchItemsByCategory(int number, BufferedWriter writer) throws Exception {
        ...
        for (int i = 0; i < number; i++) {
            ...
            String itemNum = String.valueOf(getRandomNumber(itemsNumber));
            String q1 = "select * from bidding.table4 where categories0id ='categories0id" + String.valueOf(category) + "' AND items0end_date>='items0end_date" + itemNum + "';";
    ...
    public double readViewUserInfo(int number, BufferedWriter writer) throws Exception {
        String q1 = "select * from bidding.table5 where users0id ='users0id";
        String q2 = "select comments0id, comments0rating, comments0date, comments0comment from bidding.table6 where users0id ='users0id";
        double sum = 0;
        for (int i = 0; i < number; i++) {
            long d = 0;
    ...
    public double readBuyNow(int number, BufferedWriter writer) throws Exception {
        String q1 = "select users0nickname from bidding.table5 where users0id ='users0id";
        String q2 = "select * from bidding.table3 where items0id = 'items0id";
        double sum = 0;
        for (int i = 0; i < number; i++) {
            long d = 0;
            Tuple3<Long, List<Row>, Integer> tuple3 = execRecord(q1 + String.valueOf(getRandomNumber(usersNumber)) + "'");
    ...
    public double readStoreBuyNow(int number, BufferedWriter writer) throws Exception {
        String q1 = "select items0quantity, items0nb_of_bids, items0end_date from bidding.table3 where items0id ='items0id";
        double sum = 0;
        for (int i = 0; i < number; i++) {
            long d = 0;
    ...
    public long updateItem1(Map<String, String> fields, int index) {
        ...
        // update Table 2
        String q = "select users0id, bids0id from bidding.table2 where items0id='" + itemId + "';";
        Tuple3<Long, List<Row>, Integer> tuple3 = execRecord(q);
        calculateAFM("StoreBuyNow:table2", tuple3._3());
        times += (tuple3._1());
        for (Row row : tuple3._2()) {
            q = "update bidding.table2 set items0end_date='" + fields.get("items0end_date") + "' where items0id='" + itemId
        ...
        // update: Table 3 
        String items0end_date = fields.get("items0end_date");
        q = "update bidding.table3 set items0quantity='" + fields.get("items0quantity") + "', items0nb_of_bids='" + fields.get("items0nb_of_bids") + "' where items0id='" + itemId + "' AND items0end_date='" + items0end_date + "';";
        calculateAFM("StoreBuyNow:table3", 1);
        times += (UpdateQuery(q));

        // table 4
        q = "select categories0id,items0end_date from bidding.stable15 where items0id = '" + itemId + "';";
        tuple3 = execRecord(q);
        calculateAFM("StoreBuyNow:stable15", tuple3._3());
        times += (tuple3._1());
        for (Row row : tuple3._2()) {
            String cat0id = row.getString("categories0id");
            q = "update bidding.table4 set items0quantity='" + fields.get("items0quantity") + "', items0nb_of_bids='" + fields.get("items0nb_of_bids") + "' where categories0id='" + cat0id
                    + "' AND items0id='" + itemId + "' AND items0end_date='" + row.getString("items0end_date") + "';";
            times += (UpdateQuery(q));
            calculateAFM("StoreBuyNow:table4", 1);
        }

        // table 12
        q = "select regions0id, categories0id, users0id from bidding.stable19 where items0id = '" + itemId + "';";
        tuple3 = execRecord(q);
        calculateAFM("StoreBuyNow:stable19", tuple3._3());
        times += (tuple3._1());
        for (Row row : tuple3._2()) {
            q = "update bidding.table12 set items0quantity='" + fields.get("items0quantity") + "', items0nb_of_bids='" + fields.get("items0nb_of_bids") + "' where regions0id='" + row.getString("regions0id")
                    + "' AND categories0id='" + row.getString("categories0id")
                    + "' AND items0id ='" + itemId
                    + "' AND users0id='" + row.getString("users0id")
                    + "' AND items0end_date='" + items0end_date + "';";
        ...
        if (index == 1) {
            // table 15
            q = "select categories0id from bidding.stable15 where items0id = '" + itemId + "';";
            tuple3 = execRecord(q);
            calculateAFM("StoreBuyNow:stable15", tuple3._3());
            times += (tuple3._1());
            for (Row row : tuple3._2()) {
                q = "update bidding.stable15 set items0end_date='" + items0end_date + "' where items0id='" + itemId
                        + "' AND categories0id='" + row.getString("categories0id") + "';";
                times += (UpdateQuery(q));
            ...
            times += (tuple3._1());
            for (Row row : tuple3._2()) {
                q = "update bidding.stable19 set items0end_date='" + items0end_date + "' where items0id='" + itemId
                        + "' AND  regions0id='" + row.getString("regions0id")
                        + "' AND  categories0id='" + row.getString("categories0id")
                        + "' AND  users0id='" + row.getString("users0id") + "';";
                times += (UpdateQuery(q));
    ...
    public double readPutBid(int number, BufferedWriter writer) throws Exception {
        String q1 = "select users0nickname from bidding.table5 where users0id ='users0id";
        String q2 = "select * from bidding.table3 where items0id = 'items0id";
        String q3 = "select bids0id from bidding.table2 where items0id = 'items0id";
        double sum = 0;
        for (int i = 0; i < number; i++) {
            long d = 0;
            Tuple3<Long, List<Row>, Integer> tuple3 = execRecord(q1 + String.valueOf(getRandomNumber(usersNumber)) + "'");
            ...
            d += tuple3._1();
            sum += tuple3._1();
            tuple3 = execRecord(q3 + String.valueOf(itemId) + "'");
            calculateAFM("PutBid:table2", tuple3._3());
            d += tuple3._1();
            sum += tuple3._1();
            for (Row row : tuple3._2()) {
                String q4 = "select bids0qty, bids0date from bidding.mv_table22 where bids0id ='"
                        + row.getString("bids0id") + "';"; //+ "' AND bids0date='" + row.getString("bids0date")
                //+ "' AND bids0bid='" + row.getString("bids0bid") + "';";
    ...
    public double readStoreBid(int number, BufferedWriter writer) throws Exception {
        ...
        d += l;
        sum += l;
        String q1 = "select items0nb_of_bids, items0max_bid,items0end_date from bidding.table3 where items0id ='items0id";
        for (int i = 0; i < number; i++) {
            Tuple3<Long, List<Row>, Integer> tuple3 = execRecord(q1 + items0id + "'");
    ...
    public double readPutComment(int number, BufferedWriter writer) throws Exception {
        String q1 = "select users0nickname, users0password from bidding.table5 where users0id ='users0id";
        String q2 = "select * from bidding.table3 where items0id ='items0id";
        String q3 = "select * from bidding.table5 where users0id ='users0id";
        double sum = 0;
        for (int i = 0; i < number; i++) {
            long d = 0;
            String userId = String.valueOf(getRandomNumber(usersNumber));
    ...
    public double readStoreComment(int number, BufferedWriter writer) throws Exception {
        String q1 = "select users0rating from bidding.table5 where users0id ='users0id";
//        BufferedWriter writer = new BufferedWriter(new FileWriter(baseAddress + "/StoreComment.txt"));
        double sum = 0;
        for (int i = 0; i < number; i++) {
            long d = 0;
            ...
            for (Row row : tuple3._2()) {
                ...
                String q = "update bidding.table5 set users0rating='" + users0rating + "' where users0id='users0id" + user0id + "';";
    ...
    public double readAboutMe(int number, BufferedWriter writer) throws Exception {
        String q1 = "select * from bidding.table5 where users0id ='users0id";
        String q2 = "select * from bidding.table6 where users0id ='users0id";
        String q3 = "select users0nickname from bidding.table8 where comments0id ='comments0id";

        String q4 = "select * from bidding.table9 where users0id='users0id";
        String q41 = "select * from bidding.table3 where items0id='";

        String q5 = "select items0id from bidding.table10 where users0id ='users0id";
        String q51 = "select * from bidding.table3 where items0id ='";

        String q6 = "select items0id from bidding.table11 where users0id = 'users0id";
        String q61 = "select * from bidding.table3 where items0id = '";
        double sum = 0;
        for (int i = 0; i < number; i++) {
            long d = 0;
            ...
            sum += tuple3._1();
            for (Row row : tuple3._2()) {
                Tuple3<Long, List<Row>, Integer> tmpTuple = execRecord(q41 + row.getString("items0id") + "'");
    ...
    public double readSearchItemsByRegion(int number, BufferedWriter writer) throws Exception {
        ...
        for (int i = 0; i < number; i++) {
            ...
            Tuple3<Long, List<Row>, Integer> tuple3 = execRecord("select * from bidding.table12 where regions0id = '" + regions0id + "' and categories0id = '" + categories0id + "'");
    ...
    public double readBrowseRegions(int number, BufferedWriter writer) throws Exception {
        String q1 = "select regions0id, regions0name from bidding.table13 where regions0dummy = '0'";
        double sum = 0;
        for (int i = 0; i < number; i++) {
            Tuple3<Long, List<Row>, Integer> tuple3 = execRecord(q1);
